  ...
  return (
    <div className="min-h-screen w-full overflow-hidden bg-black text-white relative font-sans selection:bg-brand-pink selection:text-white">
      ...
      <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-center z-50 mix-blend-difference">
        <h1 className="font-black tracking-tighter text-2xl">THE IDEA <span className="text-xs align-top opacity-50">BETA</span></h1>
        <button onClick={switchScene} className="hover:rotate-180 transition-transform duration-500">
           <RefreshCw size={24} />
        </button>
      </div>
...
function VinylScene({ isGodMode }: { isGodMode: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* The Turntable */}
      <div className="relative z-10 scale-75 md:scale-100 cursor-pointer group" onClick={() => setIsPlaying(!isPlaying)}>
        {/* Tonearm */}
        <motion.div 
      ...
      <div className="max-w-3xl space-y-2">
        {lines.map((line, i) => (
          <div key={i} className="flex gap-2">
            ...
            <span className={line.includes("ERROR") ? "text-red-500" : "text-brand-yellow"}>{line}</span>
          </div>
        ))}
